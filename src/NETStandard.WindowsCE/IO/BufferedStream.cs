...
namespace System.IO
{
    ...
    /// </summary>
    public sealed class BufferedStream : AsyncStream
    {
        ...
        private AsyncStream _stream;                             // Underlying stream.  Close sets _stream to null.
        ...
        public BufferedStream(AsyncStream stream)
            : this(stream, DefaultBufferSize)
        {
        }

        public BufferedStream(Stream stream, int bufferSize)
            : this(stream?.ConvertToAsync(), bufferSize)
        {
        }

        public BufferedStream(AsyncStream stream, int bufferSize)
        {
            if (stream == null)
        ...
        public AsyncStream UnderlyingStream
        {
            get
            {
                return _stream;
        ...
        public override Task<int> ReadAsync(byte[] buffer, int offset, int count, CancellationToken cancellationToken)
        {
            ...
            return ReadFromUnderlyingStreamAsync(
                buffer, offset + bytesFromBuffer, count - bytesFromBuffer,
        ...
        private async ValueTask<int> ReadFromUnderlyingStreamAsync(
            byte[] array, int offset, int count, CancellationToken cancellationToken, int bytesAlreadySatisfied, Task semaphoreLockTask)
        {
...
#region Write algorithm comment
        ...
        private async Task WriteToUnderlyingStreamAsync(
            byte[] array, int offset, int count, CancellationToken cancellationToken, Task semaphoreLockTask)
        {
            Debug.Assert(array != null);
            Debug.Assert(offset >= 0);
            Debug.Assert(count >= 0);
