...
namespace System
{
    internal static partial class SpanHelpers
    {
        ...
        public static unsafe void ClearPointerSizedWithoutReferences(ref byte b, UIntPtr byteLength)
        {
            ...
            var i = IntPtr.Zero;
            while (i.LessThanEqual(UIntPtr2.Subtract(byteLength, sizeof(Reg64))))
            {
                Unsafe.As<byte, Reg64>(ref Unsafe.Add<byte>(ref b, i)) = default(Reg64);
                i = IntPtr2.Add(i, sizeof(Reg64));
            }
            if (i.LessThanEqual(UIntPtr2.Subtract(byteLength, sizeof(Reg32))))
            {
                Unsafe.As<byte, Reg32>(ref Unsafe.Add<byte>(ref b, i)) = default(Reg32);
                i = IntPtr2.Add(i, sizeof(Reg32));
            }
            if (i.LessThanEqual(UIntPtr2.Subtract(byteLength, sizeof(Reg16))))
            {
                Unsafe.As<byte, Reg16>(ref Unsafe.Add<byte>(ref b, i)) = default(Reg16);
                i = IntPtr2.Add(i, sizeof(Reg16));
            }
            if (i.LessThanEqual(UIntPtr2.Subtract(byteLength, sizeof(long))))
            {
                Unsafe.As<byte, long>(ref Unsafe.Add<byte>(ref b, i)) = 0;
                i = IntPtr2.Add(i, sizeof(long));
            }
            // JIT: Should elide this if 64-bit
            if (sizeof(IntPtr) == sizeof(int))
            {
                if (i.LessThanEqual(UIntPtr2.Subtract(byteLength, sizeof(int))))
                {
                    Unsafe.As<byte, int>(ref Unsafe.Add<byte>(ref b, i)) = 0;
                    i = IntPtr2.Add(i, sizeof(int));
                }
            }
        }
        ...
        public static void ClearPointerSizedWithReferences(ref IntPtr ip, UIntPtr pointerSizeLength)
        {
            ...
            var i = IntPtr.Zero;
            var n = IntPtr.Zero;
            while ((n = IntPtr2.Add(i, 8)).LessThanEqual(pointerSizeLength))
            {
                Unsafe.Add<IntPtr>(ref ip, IntPtr2.Add(i, 0)) = default(IntPtr);
                Unsafe.Add<IntPtr>(ref ip, IntPtr2.Add(i, 1)) = default(IntPtr);
                Unsafe.Add<IntPtr>(ref ip, IntPtr2.Add(i, 2)) = default(IntPtr);
                Unsafe.Add<IntPtr>(ref ip, IntPtr2.Add(i, 3)) = default(IntPtr);
                Unsafe.Add<IntPtr>(ref ip, IntPtr2.Add(i, 4)) = default(IntPtr);
                Unsafe.Add<IntPtr>(ref ip, IntPtr2.Add(i, 5)) = default(IntPtr);
                Unsafe.Add<IntPtr>(ref ip, IntPtr2.Add(i, 6)) = default(IntPtr);
                Unsafe.Add<IntPtr>(ref ip, IntPtr2.Add(i, 7)) = default(IntPtr);
                i = n;
            }
            if ((n = IntPtr2.Add(i, 4)).LessThanEqual(pointerSizeLength))
            {
                Unsafe.Add<IntPtr>(ref ip, IntPtr2.Add(i, 0)) = default(IntPtr);
                Unsafe.Add<IntPtr>(ref ip, IntPtr2.Add(i, 1)) = default(IntPtr);
                Unsafe.Add<IntPtr>(ref ip, IntPtr2.Add(i, 2)) = default(IntPtr);
            ...
            if ((n = IntPtr2.Add(i, 2)).LessThanEqual(pointerSizeLength))
            {
                Unsafe.Add<IntPtr>(ref ip, IntPtr2.Add(i, 0)) = default(IntPtr);
                Unsafe.Add<IntPtr>(ref ip, IntPtr2.Add(i, 1)) = default(IntPtr);
                i = n;
            }
            if ((IntPtr2.Add(i, 1)).LessThanEqual(pointerSizeLength))
            {
