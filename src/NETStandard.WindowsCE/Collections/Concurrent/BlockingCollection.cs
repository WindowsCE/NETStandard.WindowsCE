...
namespace System.Collections.Concurrent
{
    ...
    public class BlockingCollection<T> : IEnumerable<T>, ICollection, IDisposable, IReadOnlyCollection<T>
    {
        ...
        public BlockingCollection(IProducerConsumerCollection<T> collection, int boundedCapacity)
        {
            ...
            {
                throw new ArgumentOutOfRangeException2(nameof(boundedCapacity), boundedCapacity, SR.BlockingCollection_ctor_BoundedCapacityRange);
            }
            if (collection == null)
            {
            ...
            if (waitForSemaphoreWasSuccessful)
            {
                ...
                try
                {
                    ...
                    if (!removeSucceeded)
                    {
                        ...
                        throw new InvalidOperationException(SR.BlockingCollection_Take_CollectionModified);
                    }
                }
                finally
                {
        ...
        private static int TryAddToAnyCore(BlockingCollection<T>[] collections, T item, int millisecondsTimeout, CancellationToken externalCancellationToken)
        {
            ...
            while (millisecondsTimeout == Timeout.Infinite || timeout >= 0)
            {
                ...
                using (CancellationTokenSource linkedTokenSource = CancellationTokenSource.CreateLinkedTokenSource(collatedCancellationTokens))
                {
                    ...
                    index = WaitHandle2.WaitAny(handles.ToArray(), timeout);
                ...
                Debug.Assert((index == WaitHandle2.WaitTimeout) || (index >= 0 && index < handles.Count));

                if (index == WaitHandle2.WaitTimeout) //case#2
...
#pragma warning restore 0420
        ...
        void ICollection.CopyTo(Array array, int index)
        {
            ...
            {
                throw new ArgumentOutOfRangeException2(nameof(index), index, SR.BlockingCollection_CopyTo_NonNegative);
            }
